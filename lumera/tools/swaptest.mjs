...
for (const [w, h, tag] of [[1440, 900, "desk"], [390, 844, "mob"]]) {
  ...
  ok(shape.length === 2, `${tag} two swapping figures (${shape.length})`);
  ok(shape.every(s => s.ims === 2 && s.dt), `${tag} each carries a second frame`);
  ok(shape.every(s => s.dots === 2), `${tag} each shows the two-dot tell`);
  ok(shape.filter(s => s.light).length === 1, `${tag} only the piece shot on white gets the light ground (${shape.filter(s => s.light).length})`);
  ...
  ok(!seen.has(2), `${tag} never all at once (${[...seen].sort().join(",")})`);
