...
for (const [w, h, tag] of [[1440, 900, "desk"], [390, 844, "mob"]]) {
  ...
  ok(shape.length === 3, `${tag} three swapping figures (${shape.length})`);
  ...
  ok(!seen.has(3), `${tag} never all three at once (${[...seen].sort().join(",")})`);
  ok(seen.has(1), `${tag} the rail does change (${[...seen].sort().join(",")})`);
