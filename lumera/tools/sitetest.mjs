...
for (const [tag, vp] of [["desk",{width:1440,height:900}],["mob",{width:390,height:844}]]) {
  ...
  // compass
  await p.evaluate(() => window.scrollTo({top: scrollY + document.getElementById("clients").getBoundingClientRect().top, behavior:"instant"})); for (let i = 0; i < 30 && !/clients|לקוחות/i.test(await p.evaluate(() => document.getElementById("whereT").textContent)); i++) await p.waitForTimeout(250);
  out.push((await p.evaluate(() => document.getElementById("whereT").textContent === "For clients" && document.getElementById("whereN").textContent === "05") ? "PASS":"FAIL") + ` ${tag} compass says where you are (${await p.evaluate(() => document.getElementById("whereN").textContent + " " + document.getElementById("whereT").textContent)})`);
  ...
  out.push(((await p.evaluate(() => ["hero","house","inside","craft","what","collection","macro","build","wrist","clients","concierge","end"].join() === [...document.querySelectorAll("main > section")].map(s => s.id).join())) ? "PASS":"FAIL") + ` ${tag} journey order`);
