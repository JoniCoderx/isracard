...
for (const [tag, vp] of [["desk",{width:1440,height:900}],["mob",{width:390,height:844}]]) {
  ...
  out.push(((await p.evaluate(() => (document.querySelector(".sh .mark .lg") || {}).textContent === "SILAVU" && document.querySelectorAll(".sh .mark svg.sy path").length === 1 && (function () { var ps = [].slice.call(document.querySelectorAll("#intro svg.sy > path")); return ps.length === 3 && ps.every(function (x) { return x.getAttribute("d") === ps[0].getAttribute("d") && x.getAttribute("d").length > 20000; }); })() && document.querySelectorAll("#end svg.sy.huge").length === 1)) ? "PASS":"FAIL") + ` ${tag} symbol, lockup, intro trace, footer symbol`);
