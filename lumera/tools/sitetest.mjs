...
for (const [tag, vp] of [["desk",{width:1440,height:900}],["mob",{width:390,height:844}]]) {
  ...
  out.push(((await p.evaluate(() => ["hero","house","inside","craft","what","collection","bespoke","macro","build","wrist","clients","concierge","end"].join() === [...document.querySelectorAll("main > section")].map(s => s.id).join())) ? "PASS":"FAIL") + ` ${tag} journey order`);
