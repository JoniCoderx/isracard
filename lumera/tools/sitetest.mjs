...
for (const [tag, vp] of [["desk",{width:1440,height:900}],["mob",{width:390,height:844}]]) {
  ...
  // compass
  await p.evaluate(() => window.scrollTo({top: scrollY + document.getElementById("clients").getBoundingClientRect().top, behavior:"instant"})); for (let i = 0; i < 30 && !/how it works|איך זה עובד/i.test(await p.evaluate(() => document.getElementById("whereT").textContent)); i++) await p.waitForTimeout(250);
  out.push((await p.evaluate(() => document.getElementById("whereT").textContent === "How it works" && document.getElementById("whereN").textContent === "05") ? "PASS":"FAIL") + ` ${tag} compass says where you are (${await p.evaluate(() => document.getElementById("whereN").textContent + " " + document.getElementById("whereT").textContent)})`);
  ...
  if (vp.width >= 1000) { await p.evaluate(() => { const h = document.getElementById("hpin"); window.scrollTo({top: scrollY + h.getBoundingClientRect().top + (h.offsetHeight - innerHeight) * 0.6, behavior:"instant"}); }); for (let i = 0; i < 60 && !(await p.evaluate(() => Math.abs(new DOMMatrix(getComputedStyle(document.getElementById("htrack")).transform).m41) > 200)); i++) await p.waitForTimeout(100);
    out.push(((await p.evaluate(() => { const t = document.getElementById("htrack"); const m = new DOMMatrix(getComputedStyle(t).transform); const h = document.getElementById("hpin"); return h.offsetHeight > innerHeight * 1.5 && Math.abs(m.m41) > 200 && document.querySelectorAll(".htrack .piece").length === 6; })) ? "PASS":"FAIL") + ` ${tag} collection rail moves sideways with scroll (${await p.evaluate(() => getComputedStyle(document.getElementById("htrack")).transform + " h=" + document.getElementById("hpin").offsetHeight + " y=" + scrollY + " top=" + Math.round(document.getElementById("hpin").getBoundingClientRect().top) + " body=" + document.body.className + " dir=" + document.documentElement.dir)})`); }
  ...
  out.push(((await p.evaluate(() => ["hero","house","inside","craft","what","collection","bespoke","macro","build","wrist","clients","film","concierge","end"].join() === [...document.querySelectorAll("main > section")].map(s => s.id).join())) ? "PASS":"FAIL") + ` ${tag} journey order`);
