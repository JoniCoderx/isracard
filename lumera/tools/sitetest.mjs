...
for (const [tag, vp] of [["desk",{width:1440,height:900}],["mob",{width:390,height:844}]]) {
  ...
  for (let i = 0; i < 40 && !(await p.evaluate(() => window.__stone && window.__stone.ok && window.__stone.p > 0.3 && window.__stone.p < 0.5)); i++) await p.waitForTimeout(100);
  out.push(((await p.evaluate(() => window.__stone.ok && window.__stone.p > 0.3 && window.__stone.p < 0.5 && document.getElementById("whereN").textContent === "02" && +document.querySelectorAll("#beats .beat")[1].style.opacity > 0.5)) ? "PASS":"FAIL") + ` ${tag} pinned stone turns with scroll (p=${await p.evaluate(() => window.__stone.p.toFixed(2))})`);
  out.push(((await p.evaluate(() => { const c = document.getElementById("stonecv"); const g = c.getContext("webgl"); const px = new Uint8Array(4); g.readPixels(c.width >> 1, c.height >> 1, 1, 1, g.RGBA, g.UNSIGNED_BYTE, px); return px[3] > 0; })) ? "PASS":"FAIL") + ` ${tag} stone actually drawn`);
  // stone picker
  ...
  out.push((await p.evaluate(() => document.body.scrollHeight / innerHeight < (innerWidth < 1000 ? 20 : 18)) ? "PASS":"FAIL") + ` ${tag} page length ${await p.evaluate(() => (document.body.scrollHeight / innerHeight).toFixed(1))} screens`);
